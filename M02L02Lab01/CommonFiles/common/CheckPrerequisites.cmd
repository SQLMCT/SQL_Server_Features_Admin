IF "%SQLSERVER%"=="" (
  ...
  SET SQLSERVER=(SQLN1\SQL2022)
)

REM Check for connectivity
sqlcmd.exe -E -S SQLN1\SQL2022 -Q"select 1"  > NUL 2>&1
IF "%ERRORLEVEL%" NEQ "0" (
  @ECHO.
...
sqlcmd.exe -E -S SQLN1\SQL2022 -Q"exit (select count(*) from master..sysdatabases where name = 'AdventureWorksPTO')" > NUL 2>&1
...
@ECHO You can manually install "AdventureWorks2022 Database and rename it AdventureWorksPTO" from: 
